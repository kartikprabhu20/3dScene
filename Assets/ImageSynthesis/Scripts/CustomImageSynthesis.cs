...
public class CustomImageSynthesis : MonoBehaviour
{
    ...
    public CustomImageSynthesis(Camera mainCam)
    {
        mainCamera = mainCam;
    ...
    {
        int targetDisplay = 1;
        var mainCam = mainCamera.GetComponent<Camera>();
        foreach (var pass in capturePasses)
        {
            if (pass.camera == mainCam)
                continue;
    ...
    public IEnumerator Save(string filename, GameObject room, GameObject model, int width = -1, int height = -1, string path = "")
    {
        if (width <= 0 || height <= 0)
        {
            width = Screen.width;
        ...
        return WaitForEndOfFrameAndSave(pathWithoutExtension, filenameExtension, width, height, room, model);
    }

    private IEnumerator WaitForEndOfFrameAndSave(string filenameWithoutExtension, string filenameExtension, int width, int height, GameObject room, GameObject model)
    {
        yield return new WaitForEndOfFrame();
        Save(filenameWithoutExtension, filenameExtension, width, height, room, model);
    }

    private void Save(string filenameWithoutExtension, string filenameExtension, int width, int height, GameObject room, GameObject model)
    {
        foreach (var pass in capturePasses)
        {
            Debug.Log(pass.name);
    ...
    private void Save(Camera cam, string filename, int width, int height, bool supportsAntialiasing, bool needsRescale)
    {
        ...
        var renderRT = (!needsRescale) ? finalRT :
            RenderTexture.GetTemporary(mainCam.pixelWidth, mainCam.pixelHeight, depth, format, readWrite, antiAliasing);
